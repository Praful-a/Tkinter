...
def search_customer():
    ...
    search_customers.geometry("1050x700")

    def update():
        sql_command = """Update customers SET first_name = %s, last_name = %s, zipcode=%s, price_paid = %s, email = %s, address_1 = %s, city = %s, state = %s, country = %s, phone = %s, payment_method = %s, discount_code = %s WHERE user_id = %s"""
        ...
        inputs = (first_name, last_name, zipcode, price_paid, email, address_1, city,
                  state, country, phone, payment_method, discount_code, id_value)
    ...
    def edit_now(id, index):
        ...
        first_name_label = Label(search_customers, text="First Name").grid(
            row=index+1, column=0, sticky=W, padx=10)
        last_name_label = Label(search_customers, text="Last Name").grid(
            row=index+2, column=0, sticky=W, padx=10)
        address1_label = Label(search_customers, text="Address").grid(
            row=index+3, column=0, sticky=W, padx=10)
        city_label = Label(search_customers, text="City").grid(
            row=index+4, column=0, sticky=W, padx=10)
        state_label = Label(search_customers, text="State").grid(
            row=index+5, column=0, sticky=W, padx=10)
        zipcode_label = Label(search_customers, text="Zipcode").grid(
            row=index+6, column=0, sticky=W, padx=10)
        country_label = Label(search_customers, text="Country").grid(
            row=index+7, column=0, sticky=W, padx=10)
        Phone_label = Label(search_customers, text="Phone Number").grid(
            row=index+8, column=0, sticky=W, padx=10)
        email_label = Label(search_customers, text="Email Address").grid(
            row=index+9, column=0, sticky=W, padx=10)
        payment_label = Label(search_customers, text="Payment Method").grid(
            row=index+10, column=0, sticky=W, padx=10)
        discount_code_label = Label(search_customers, text="Discount Code").grid(
            row=index+11, column=0, sticky=W, padx=10)
        price_paid_label = Label(search_customers, text="Price Paid").grid(
            row=index+12, column=0, sticky=W, padx=10)
        id_label = Label(search_customers, text="User ID").grid(
            row=index+13, column=0, sticky=W, padx=10)
        ...
        city_box2.grid(row=index+4, column=1, pady=5)
        city_box2.insert(0, result2[0][7])

        global state_box2
        state_box2 = Entry(search_customers)
        state_box2.grid(row=index+5, column=1, pady=5)
        state_box2.insert(0, result2[0][8])

        global zipcode_box2
        zipcode_box2 = Entry(search_customers)
        zipcode_box2.grid(row=index+6, column=1, pady=5)
        zipcode_box2.insert(0, result2[0][3])

        global country_box2
        country_box2 = Entry(search_customers)
        country_box2.grid(row=index+7, column=1, pady=5)
        country_box2.insert(0, result2[0][9])

        global phone_box2
        phone_box2 = Entry(search_customers)
        phone_box2.grid(row=index+8, column=1, pady=5)
        phone_box2.insert(0, result2[0][10])

        global email_box2
        email_box2 = Entry(search_customers)
        email_box2.grid(row=index+9, column=1, pady=5)
        email_box2.insert(0, result2[0][5])

        global payment_method_box2
        payment_method_box2 = Entry(search_customers)
        payment_method_box2.grid(row=index+10, column=1, pady=5)
        payment_method_box2.insert(0, result2[0][11])

        global discount_code_box2
        discount_code_box2 = Entry(search_customers)
        discount_code_box2.grid(row=index+11, column=1, pady=5)
        discount_code_box2.insert(0, result2[0][12])

        global price_paid_box2
        price_paid_box2 = Entry(search_customers)
        price_paid_box2.grid(row=index+12, column=1, pady=5)
        price_paid_box2.insert(0, result2[0][3])

        global id_box2
        id_box2 = Entry(search_customers)
        id_box2.grid(row=index+13, column=1, pady=5)
        ...
        if not result:
            ...
            for index, x in enumerate(result):
                ...
                edit_button = Button(search_customers, text="Edit ",
                                     command=lambda: edit_now(id_reference, index))
...
address1_label = Label(root, text="Address").grid(row=3, column=0, sticky=W, padx=10)
...
country_label = Label(root, text="Country").grid(row=7, column=0, sticky=W, padx=10)
...
payment_label = Label(root, text="Payment Method").grid(row=10, column=0, sticky=W, padx=10)
...
country_box.grid(row=7, column=1, pady=5)
...
# Search customers
search_customers_button = Button(root, text="Search\Edit Cutomers", command=search_customer)
